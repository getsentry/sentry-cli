...
fn find_matching_head_commit(spec: &CommitSpec, repos: &[Repo], disable_discovery: bool) -> Result<String>
{
...
/// from it.
pub fn find_head_commits(specs: Vec<CommitSpec>, repos: Vec<Repo>)
    -> Result<Vec<HeadCommit>>
{
    ...
    for spec in &specs {
        let head = if let Some(ref rev) = spec.rev {
            ...
            find_matching_head_commit(&spec, &repos[..], specs.len() == 1)?
        };
        rv.push(HeadCommit {
