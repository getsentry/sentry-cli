...
impl AdaptiveSampler {
    ...
    fn record_sampled_event(&mut self) {
        ...
        self.recent_events.retain(|(timestamp, _)| *timestamp > cutoff);
    }
    ...
    fn adjust_sampling_rate(&mut self) {
        ...
        if current_eps > self.target_eps * 1.2 {
            ...
            debug!("High load detected ({:.1} eps), reducing sampling rate: {:.3} -> {:.3}", 
            ...
            self.current_rate = (self.current_rate * 1.2).min(self.base_rate).min(self.max_rate);
            debug!("Low load detected ({:.1} eps), increasing sampling rate: {:.3} -> {:.3}", 
                   current_eps, previous_rate, self.current_rate);
        }
    }
    ...
    fn calculate_current_eps(&self) -> f64 {
        ...
        let now = Instant::now();
        let events_in_window: usize = self.recent_events
            .iter()
