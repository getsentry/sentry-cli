...
impl LogBatch {
    ...
    pub fn new_adaptive(max_size: usize, timeout: Duration, adaptive_config: AdaptiveBatchingConfig) -> Self {
        LogBatch {
    ...
    pub fn add_entry(&mut self, entry: String) -> bool {
        ...
        debug!("Added log entry to batch ({}/{})", self.entries.len(), self.max_size);
    ...
    fn get_adaptive_batch_size(&self) -> usize {
        ...
        if recent_times.len() >= 3 {
            ...
            let avg_recent_interval = last_intervals.iter().sum::<Duration>() / last_intervals.len() as u32;
