...
impl PositionTracker {
    ...
    pub fn new<P: AsRef<Path>>(file_path: P) -> Result<Self> {
        ...
        debug!("Initialized position tracker for {} at position {} (size: {})", 
               file_path.display(), current_size, current_size);
    ...
    pub fn check_for_new_data(&mut self) -> Result<u64> {
        ...
        if new_size < self.current_size {
            debug!("File truncation detected: {} -> {} bytes", 
    ...
    pub fn read_new_lines(&mut self) -> Result<Vec<String>> {
        ...
        debug!("Read {} new lines from {}", 
               lines.len(), self.file_path.display());
