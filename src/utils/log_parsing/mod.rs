...
pub fn detect_log_format(sample_lines: &[String]) -> LogFormat {
    ...
    for line in sample_lines.iter().take(10) { // Check first 10 lines
...
mod tests {
    ...
    fn test_log_level_from_str() {
        assert_eq!(LogLevel::from_str("ERROR").unwrap().to_sentry_level(), "error");
        assert_eq!(LogLevel::from_str("info").unwrap().to_sentry_level(), "info");
        assert_eq!(LogLevel::from_str("WARN").unwrap().to_sentry_level(), "warning");
