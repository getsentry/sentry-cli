...
impl PlainParser {
    ...
    fn extract_level(&self, line: &str) -> Option<LogLevel> {
        if let Some(level_match) = self.level_regex.find(line) {
            ...
            if line_lower.contains("exception") || line_lower.contains("failed") || line_lower.contains("error") {
                Some(LogLevel::Error)
...
mod tests {
    ...
    fn test_extract_timestamp() {
        ...
        assert!(parser.extract_timestamp("2023-12-25 10:00:00 message").is_some());
        assert!(parser.extract_timestamp("2023/12/25 10:00:00 message").is_some());
        assert!(parser.extract_timestamp("Dec 25 10:00:00 message").is_some());
    ...
    fn test_extract_level() {
        ...
        assert!(matches!(parser.extract_level("ERROR: something failed"), Some(LogLevel::Error)));
        assert!(matches!(parser.extract_level("WARNING: be careful"), Some(LogLevel::Warning)));
        assert!(matches!(parser.extract_level("INFO: normal operation"), Some(LogLevel::Info)));
        assert!(matches!(parser.extract_level("DEBUG: detailed info"), Some(LogLevel::Debug)));
        assert!(matches!(parser.extract_level("exception occurred"), Some(LogLevel::Error)));
        assert!(matches!(parser.extract_level("plain message"), Some(LogLevel::Info)));
    }
