//! Command for sending Apple crash reports to Sentry.
//!
//! This command allows users to manually upload Apple crash reports in .ips (JSON) format
//! to Sentry for symbolication and analysis.
...
pub fn make_command(command: Command) -> Command {
    command
        ...
        .long_about(
            "Send Apple crash reports (.ips) to Sentry.\n\n\
             This command parses Apple crash report files in .ips (JSON) format \
             and sends them to Sentry as error events. Sentry will automatically \
             symbolicate the crash reports if matching debug symbols (dSYMs) have \
             been uploaded.\n\n\
...
pub fn execute(matches: &ArgMatches) -> Result<()> {
    ...
    for path in paths {
        ...
        println!("Crash from file {} dispatched: {event_id}", path.display());
