...
use crate::utils::file_watcher::{FileEvent, LogFileWatcher, PositionTracker, setup_signal_handlers};
use crate::utils::log_parsing::{create_parser, detect_log_format, LogEntry, LogFormat as ParserLogFormat};
...
pub(super) fn execute(args: TailLogsArgs) -> Result<()> {
    ...
    info!("Format: {:?}", args.format.as_ref().unwrap_or(&LogFormat::Auto));
    ...
    loop {
        ...
        if shutdown_receiver.try_recv().is_ok() {
            ...
            if !log_batch.is_empty() {
                ...
                if let Err(e) = send_log_entries(&log_transmitter, &mut rate_limiter, remaining_entries) {
        ...
        match file_watcher.check_events(poll_interval)? {
                    ...
                    match parser.parse_line(&line) {
                        Ok(log_entry) => {
                            ...
                            let should_process = if let Some(ref mut sampler) = sampler {
                                let level_str = log_entry.level.as_ref().map(|l| l.to_sentry_level());
                            ...
                            if should_process {
                                ...
                                if should_flush {
                                    ...
                                    if let Err(e) = send_log_entries_with_monitoring(&log_transmitter, &mut rate_limiter, entries, &memory_monitor) {
            ...
            None => {
                ...
                if log_batch.should_flush() && !log_batch.is_empty() {
                    let entries = log_batch.flush();
                    info!("Timeout flush: sending batch of {} log entries to Sentry", entries.len());
...
fn serialize_log_entry(entry: &LogEntry) -> Result<String> {
    serde_json::to_string(entry).map_err(|e| anyhow::anyhow!("Failed to serialize log entry: {}", e))
}
...
fn send_log_entries(
    ...
    if !rate_limiter.can_send() {
        ...
        warn!("Rate limit exceeded: {}/{} events this minute. Dropping {} log entries.", 
              current, max, entries.len());
        return Ok(());
    }

    match transmitter.send_log_batch(entries) {
        Ok(event_ids) => {
            debug!("Successfully sent {} log entries to Sentry", event_ids.len());
            Ok(())
        }
        Err(e) => {
