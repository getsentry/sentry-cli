...
impl Api {
    ...
    pub fn get_sourcemap_processing_issues(
        &self,
        org: &str,
        project: &str,
        event_id: &str,
        ...
        if resp.status() == 404 {
            ...
            let resp: SourceMapProcessingIssueResponse = resp.convert()?;
            Ok(Some(resp.errors))
        }
    }
}
...
pub enum SourceMapProcessingIssue {
    UnknownError,
    #[serde(rename = "no_release_on_event")]
    MissingRelease,
    #[serde(rename = "no_user_agent_on_release")]
    MissingUserAgent,
    #[serde(rename = "no_sourcemaps_on_release")]
    MissingSourcemaps,
    UrlNotValid,
    NoUrlMatch,
    #[serde(rename_all = "camelCase")]
...
impl fmt::Display for SourceMapProcessingIssue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NoUrlMatch => {
                f.write_str("The absolute path url does not match any source maps.")
            }
            ...
            Self::DistMismatch {
                ...
                write!(
                    f,
                    "The dist values do not match. Event: {event_dist}, Artifact: "
                )?;
